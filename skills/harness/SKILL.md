...
6. **Continuous Learning (learning 포함 요청 시)** — `continuous-learning` skill + `observe.py` + `compact-suggest.py` 생성. Hook 등록으로 tool 이벤트를 `~/.local/share/harness-meta-factory/` JSONL에 기록. confidence ≥ 0.7 instinct만 세션 주입 (토큰 최적화). 상세: `references/platform-learning.md` · `references/continuous-learning-templates.md`.
7. **하네스는 진화하는 시스템** — 피드백 반영, drift 감지, 지속 갱신.
...
**Claude Code:** `model: "opus"` 명시. **Cursor:** Task 전 `Read(.cursor/agents/{name}.md)` → prompt 삽입. **Codex:** spawn은 orchestrator skill에서 명시적 지시.
...
### Phase 5-6: Continuous Learning 모듈 (learning 포함 요청 시)

> 트리거: "learning 포함", "학습 기능", "continuous learning", "토큰 최적화"

`references/continuous-learning-templates.md`를 읽고 다음을 생성한다.

**생성 파일:**
```
.agents/skills/continuous-learning/
├── SKILL.md          ← templates의 SKILL.md 템플릿 사용
├── config.json       ← templates의 config.json 템플릿 사용
└── scripts/
    ├── observe.py         ← templates의 observe.py 전체 코드
    ├── instinct-cli.py    ← templates의 instinct-cli.py 전체 코드
    └── compact-suggest.py ← templates의 compact-suggest.py 전체 코드
```

플랫폼별 hook 파일에 `references/platform-learning.md`의 "플랫폼별 Hook 등록" 섹션 병합:

| 플랫폼 | hook 파일 | 주요 이벤트 |
|--------|-----------|------------|
| Claude Code | `.claude/hooks/hooks.json` | PreToolUse·PostToolUse·Stop; compact-suggest는 **PreToolUse(Edit/Write)** 이벤트에 등록 |
| Cursor | `.cursor/hooks.json` | afterFileEdit·subagentStop·stop |
| Gemini | `.gemini/settings.json` | BeforeTool·AfterTool·SessionEnd |
| Codex | `.codex/hooks.json` | PreToolUse·PostToolUse·Stop |

**Token Optimization (모든 플랫폼):**
- compact-suggest hook으로 150,000 토큰 초과 시 compact 타이밍 제안
- continuous-learning SKILL.md는 confidence ≥ 0.7 instinct만 세션 주입

**멀티플랫폼 생성 시:** `.agents/skills/continuous-learning/`에 1회 작성 후 각 플랫폼 경로에 동일 내용 배치.

**Cursor 한계 보완:** `afterFileEdit` + `stop` 기반 부분 학습 + orchestrator handoff.md에 "세션 패턴 요약" 섹션 추가.
...
- [ ] `.agents/skills/continuous-learning/SKILL.md` + `config.json`
- [ ] `scripts/observe.py` + `scripts/instinct-cli.py` + `scripts/compact-suggest.py`
- [ ] 플랫폼 hook 파일에 observe/compact-suggest 등록
- [ ] `HARNESS_LEARNING_DIR` 환경변수 설명 → entry file 또는 README에 기록
- [ ] Cursor: handoff.md "세션 패턴 요약" 섹션 추가 (full observation 불가 보완)
...
- [ ] `CLAUDE.md` 포인터
- [ ] (learning) `.claude/hooks/hooks.json` — PreToolUse·PostToolUse·Stop 등록; **compact-suggest는 PreToolUse(Edit/Write)** 이벤트에 추가
...
- [ ] (learning) `.cursor/hooks.json` — afterFileEdit·subagentStop·stop 등록
...
- [ ] `artifacts/`
- [ ] (learning) `.gemini/settings.json` — BeforeTool·AfterTool·SessionEnd 등록
...
- [ ] `artifacts/`
- [ ] (learning) `.codex/hooks.json` — PreToolUse·PostToolUse·Stop 등록
...
- **Continuous Learning 플랫폼 설계:** `references/platform-learning.md`
- **Continuous Learning 스크립트 템플릿:** `references/continuous-learning-templates.md`
