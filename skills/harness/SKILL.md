...
6. **하네스는 진화하는 시스템** — 피드백 반영, drift 감지, 지속 갱신 (`references/harness-evolution.md`).
...
- **Cursor에 TeamCreate/SendMessage 금지**
...
| 패턴 | 흐름 | 참조 |
|------|------|------|
| **에이전트 팀** (기본·Claude 전용) | TeamCreate → TaskCreate → SendMessage → 수집 | `references/orchestrator-template.md` 템플릿 A |
...
| 검증 항목 | 확인 내용 |
|----------|----------|
| **구조** | 파일 위치, frontmatter, 참조 일관성, 커맨드 미생성 |
