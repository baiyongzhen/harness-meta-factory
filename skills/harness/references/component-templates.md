...
```markdown
## Execution: Cursor Task (NOT Claude 팀 API)
...
Agent(spawn 팀원) → TaskCreate → SendMessage → collect _workspace/02_*.md
# TeamCreate는 v2.1.178에서 제거 — 팀원은 Agent 도구로 생성
# 전제: .claude/settings.json env.CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1
```
...
### Gemini (`.gemini/skills/{domain}-orchestrator/SKILL.md`)

```markdown
## Execution: Gemini Subagents (GA)
Phase별로 .gemini/agents/{name}.md 서브에이전트를 순차/병렬 위임.
서브에이전트 전용 훅 이벤트가 없으므로 각 Phase 시작/종료를 stdout 로깅.
Integrate artifacts/02_*.md → artifacts/final-report.md
```
...
```
> `version: 1`은 Cursor `hooks.json` 필수 최상위 필드. matcher는 `platform-hooks.md`와 동일하게 통일.

## Claude Settings (`.claude/settings.json`)

에이전트 팀 사용 여부에 따라 두 가지 기본 템플릿 중 하나를 선택한다.

### 에이전트 팀 모드 (Agent 도구 + SendMessage 사용 시 — 필수)
...
## Claude Hooks (`.claude/settings.json`의 `"hooks"` 키)

> 훅은 별도 `hooks.json`이 아니라 **`settings.json`의 `"hooks"` 키**에 등록한다. (`hooks/hooks.json`은 플러그인 내부 전용.) 아래 `"hooks": {...}`를 `settings.json`에 병합한다.
...
> ⚠️ CLAUDE.md·AGENTS.md와 동일하게 change log 테이블 필수 (`harness-evolution.md` §3: 모든 변경은 entry file 변경 이력에 기록). 에이전트·스킬 목록·디렉터리 트리는 넣지 않는다 — drift 원인.
...
### Gemini (`.gemini/agents/doc-writer.md` 또는 skill 역할)

Gemini는 서브에이전트(GA)를 `.gemini/agents/doc-writer.md`로 두거나, 동일 절차를 `.gemini/skills/sync-docs/SKILL.md`(아래 공통 Sync Docs Skill) 호출로 대체한다.

```markdown
---
name: doc-writer
description: Sync GEMINI.md with current code and harness after structural changes.
---

# Doc Writer

## Role
Keep GEMINI.md aligned with codebase and `.gemini/` harness.

## Owned files
| File | Trigger |
|------|---------|
| GEMINI.md | New endpoints, stack, structure (change log 갱신) |

## Style
- GEMINI.md = 트리거 + change log만. 에이전트·스킬 목록·디렉터리 트리 금지 (drift 원인).
``` (`.agents/skills/sync-docs/SKILL.md` — **필수**)
