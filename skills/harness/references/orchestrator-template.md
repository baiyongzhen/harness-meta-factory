...
> **플랫폼:** 이 템플릿은 **Claude Code** 전용이다.
> Cursor / Gemini CLI / Codex 오케스트레이터는 `references/platform-orchestration.md` + `references/component-templates.md`의 플랫폼별 orchestrator 섹션을 사용한다.
...
1. `_workspace/` 디렉토리 존재 여부 및 완료 마커 확인
2. 실행 모드 결정:
   - **`_workspace/` 미존재** → 초기 실행. Phase 1로 진행
   - **`_workspace/` 존재 + 완료 마커 없음 + 부분 수정 요청** → 부분 재실행. 해당 에이전트만 재호출하고, 기존 산출물 중 수정 대상만 덮어쓴다
   - **`_workspace/` 존재 + 완료 마커 있음 (최종 산출물 존재)** → 아카이브 후 새 실행
   - **`_workspace/` 존재 + 사용자가 "새로 시작" 명시** → 아카이브 후 새 실행
3. **아카이브 실행 (새 실행 시):**
   ```bash
...
2. 작업 디렉토리에 `_workspace/` 생성
   ...
   - **새 실행**: Phase 0에서 아카이브 완료 후 빈 `_workspace/` 재생성
...
### Phase 0: 컨텍스트 확인
Template A와 동일 — `_workspace/` 존재 여부 + 완료 마커 확인 → 초기/부분 재실행/아카이브 후 새 실행 분기.

### Phase 1: 준비
1. 입력 분석
2. `_workspace/` 생성 (초기 실행 시, 또는 Phase 0에서 아카이브 완료 후 빈 디렉터리 재생성)
