...
> ⚠️ 예시 1~5는 **Claude Code 전용** 팀 API(`SendMessage`/`TaskCreate`, `_workspace/`)와 Agent 도구 spawn을 사용한다. (`TeamCreate` 도구는 v2.1.178에서 제거 — 팀원은 Agent 도구로 spawn.) Cursor·Codex·Gemini에 적용할 때는 팀 API 대신 Task/spawn/skill-체인으로, `_workspace/`는 `artifacts/`로 전환해야 한다.

---

## 예시 1: 리서치 팀 (에이전트 팀 모드 · Claude Code 전용)
...
```
[리더/오케스트레이터]
    ├── Agent(팀원 spawn × 4) → research-team
    ├── TaskCreate(4개 조사 작업)
...
Phase 2: 팀 구성 — Agent 도구로 팀원 4명 spawn (한 응답에 병렬 호출)
  - Agent(subagent_type: "general-purpose", prompt: "당신은 official. 공식 채널 조사...")
  - Agent(subagent_type: "general-purpose", prompt: "당신은 media. 미디어/투자 동향 조사...")
  - Agent(subagent_type: "general-purpose", prompt: "당신은 community. 커뮤니티 반응 조사...")
  - Agent(subagent_type: "general-purpose", prompt: "당신은 background. 배경/경쟁 환경 조사...")
  - TaskCreate(tasks: [
...
```
Phase 1: Agent 도구로 worldbuilder, character-designer, plot-architect 팀원 spawn
...
Phase 2: Phase 1 팀원 작업 마무리 → prose-stylist를 서브 에이전트로 호출 (단독 집필이므로 팀 불필요)
         prose-stylist가 _workspace/의 3개 산출물을 Read하여 집필
         → 결과를 _workspace/02_prose_draft.md에 저장

Phase 3: 새 팀원 spawn — Agent 도구로 science-consultant, continuity-manager 생성
         (이전 Phase 팀원 작업을 마무리한 뒤 다음 Phase 팀원을 spawn)
...
## 예시 6: Doc Writer — 코드·하네스 문서 동기화 (**필수 모듈**)

### 팀 아키텍처: Pipeline 마지막 Phase (문서 동기화)
### 실행 모드: 서브 에이전트 / skill

**모든 하네스의 기본 구성.** 도메인 agent/skill과 별도로 `doc-writer` + `sync-docs`를 **항상** 생성한다.

```
[add-resource-orchestrator]
  Phase 1 설계 → Phase 2 구현 → Phase 3 테스트 → Phase 4 검증
  Phase 5 문서: sync-docs skill 또는 doc-writer (Task/spawn)
       ↓
  AGENTS.md API 테이블, .cursor/rules/*.mdc 갱신
```
...
| 구성 | 역할 |
|------|------|
| **doc-writer** (agent) | 담당 파일·체크리스트·스타일 가이드 정의 |
| **sync-docs** (skill) | 품질 점수(0–100) 평가 → 코드 대조 → 동기화 → 리포트 |

### 플랫폼별 동기화 대상

| 플랫폼 | Agent | Entry/Rules | Skill |
|--------|-------|-------------|-------|
| Cursor | `.cursor/agents/doc-writer.md` | `AGENTS.md`, `.cursor/rules/*.mdc` | `.cursor/skills/sync-docs/` |
| Claude | `.claude/agents/doc-writer.md` | `CLAUDE.md` | `.claude/skills/sync-docs/` |
| Gemini | skill 역할 | `GEMINI.md` | `.gemini/skills/sync-docs/` |
| Codex | `.codex/agents/doc-writer.toml` | `AGENTS.md` | `.agents/skills/sync-docs/` |

공통 work skill + 절차는 `.agents/skills/sync-docs/SKILL.md`에 1회 작성.

### Cursor Pipeline 연동 (add-resource-orchestrator Phase 5)

| 단계 | 담당 | 산출물 |
|------|------|--------|
| 5 문서 | `sync-docs` skill 또는 doc-writer Task | `AGENTS.md`, rules 갱신 |

```
Task(subagent_type: "generalPurpose", prompt: """
Read(.cursor/agents/doc-writer.md) 전문 삽입
입력: 방금 추가된 도메인/엔드포인트 경로
sync-docs skill 절차 따름
""")
```

### Hook — 편집 후 동기화 힌트 (Cursor **필수**)

`.cursor/hooks.json` → `afterFileEdit` → `check-doc-sync.sh` (guard-shell·lint와 함께 등록)

- `app/models/*.py`, `app/api/v1/endpoints/*.py`, `requirements.txt` 변경 시 AGENTS.md/rules 업데이트 힌트 반환

### 트리거 키워드 (sync-docs description)

- "문서 동기화", "AGENTS.md 업데이트", "rules 동기화", "sync docs"
- "새 엔드포인트 추가했어 문서 반영", "도메인 추가 후 문서"

템플릿: `references/component-templates.md` — Doc Writer / Sync Docs 섹션
...
### 통합 스킬 (오케스트레이터)
팀 전체를 조율하는 상위 스킬. **마지막 Phase는 sync-docs/doc-writer (필수).**
템플릿: `references/orchestrator-template.md` · Doc Writer: `references/team-examples.md` 예시 6
**실행 모드를 반드시 명시** — 에이전트 팀(기본) 또는 서브 에이전트.

### 필수 baseline (모든 하네스)
| Agent | Skill |
|-------|-------|
| doc-writer | sync-docs |
