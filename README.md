...
│           ├── skill-writing-guide.md      스킬 작성 가이드 (Scripts §11 Search-First 포함)
│           ├── skill-testing-guide.md      스킬 테스트 방법론
│           ├── qa-agent-guide.md           QA 에이전트 설계 가이드
│           ├── team-examples.md            팀 구성 예시 6가지
│           └── search-first-workflow.md    Search-First 5단계 코딩 워크플로우 (에이전트·스킬 통합 가이드)
│
├── scripts/
...
> **Search-First 워크플로우**(`references/search-first-workflow.md`)는 `harness` 스킬에 내장된 참조 문서입니다.
> 별도 설치 없이, 하네스가 코딩 에이전트·스킬을 생성할 때 자동으로 이 패턴을 적용합니다.
